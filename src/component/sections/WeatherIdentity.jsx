...
export default function WeatherIdentity() {
  ...
  // Precipitation chance per day (7 bars)
  const precipData = weather?.daily?.map((d) => ({
    day: new Date(d.date).toLocaleDateString('en-US', { weekday: 'short' }),
    value: d.precip ?? 0,
  })) ?? []

  // Conditions mix — count how many days fall into each category
  ...
  const PIE_CX = 110
  const PIE_CY = 110
  const GRAYS  = ['#6b6b6b', '#a3a3a3', '#d4d4d4', '#e5e5e5', '#f5f5f5']
  ...
  function buildPieSlices() {
    let startAngle = 0
    return conditionEntries.map(([label, count], i) => {
      ...
      startAngle  = end
      return { path, color: GRAYS[i % GRAYS.length], label, count }
    })
  }
          ...
          <div className="border border-neutral-200 rounded-2xl p-6">
            ...
            {loading ? (
              ...
              <svg viewBox={`0 0 ${precipData.length * 60} ${BAR_H + 24}`} className="w-full">
                {precipData.map(({ day, value }, i) => {
                  const barH = (value / maxPrecip) * BAR_H
                  const x    = i * 60 + 10
                  const y    = BAR_H - barH
                  return (
                    <g key={day}>
